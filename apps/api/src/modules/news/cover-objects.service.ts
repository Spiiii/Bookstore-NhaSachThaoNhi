...
@Injectable()
export class NewsCoverObjectsService {
  ...
  async read(key: string): Promise<{ bytes: Buffer; mime: string }> {
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'code' in error && error.code === 'ENOENT') {
        throw new BadRequestException('Cover object does not exist.');
      }
      throw error;
    }
    // Uploads owns full image decoding/validation. News additionally rejects non-image/SVG references.
