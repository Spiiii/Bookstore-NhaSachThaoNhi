...
@Injectable()
export class BrandLogoObjectsService {
  ...
  async read(key: string): Promise<{ bytes: Buffer; mime: string }> {
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'code' in error && error.code === 'ENOENT') {
        throw new BadRequestException('Logo object does not exist.');
      }
      throw error;
    }
    // Uploads owns full image decoding/validation. Brands additionally rejects non-image/SVG references.
